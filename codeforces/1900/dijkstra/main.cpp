...
void dijkstra(vector<Edge> adj[], int n, int s) {
  bool visited[n + 1];
  ...
  vector<pair<int, int>> path{make_pair(0, 1)};
  ...
  while (pq.size() > 0) {
    ...
    for (const auto &edge : adj[i]) {
      ...
      if (edge.to == n) reached = true;
      auto ndist = dist[i] + edge.weight;
      if (ndist < dist[edge.to]) {
        dist[edge.to] = ndist;
        if (path.back().first == edge.from) {
          path.back().second = edge.to;
        } else {
          path.push_back(make_pair(edge.from, edge.to));
        }
  ...
  LPI(i, 0, path.size(), 1) { cout << path[i].second << ' '; }
  cout << '\n';
  return;
}
